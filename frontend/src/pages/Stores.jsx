...
export default function Stores() {
  ...
  }, [categoryId, type, city, page, searchParams])
  ...
  const hasActiveFilters = Boolean(categoryId || type || city || searchParams.get('search'))

  const cities = ['Conakry', 'Kindia', 'Labé', 'Kankan']

  const typeOptions = [
    { val: null, label: t('storesPage.allVendors') },
    { val: 'business', label: t('storesPage.businessType') },
    { val: 'individual', label: t('storesPage.freelanceType') },
  ]
        ...
        <div className="flex flex-col gap-8 lg:flex-row">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 truncate">
                          {cat.icon && (
                            <CategoryIcon slug={cat.icon} className="h-4 w-4 shrink-0" />
                          )}
          ...
          <main className="min-w-0 flex-1">
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-neutral-100 pb-4">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-sm font-semibold text-neutral-800">
                  {t('storesPage.storeCount', { count: meta?.total ?? stores.length })}
                </span>
                ...
                {type && (
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-800">
                    {t('productsPage.typeTag', { type: type === 'individual' ? t('storeCard.freelance') : t('storeCard.business') })}
                    <button
                      type="button"
            ...
            {loading ? (
              ...
              <EmptyState
                title={t('storesPage.noStoresFound')}
